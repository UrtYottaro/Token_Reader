...
### 方法1: ワンライナーで自動インストール（推奨）

Terminalに以下を貼り付けるだけで、ダウンロードからセットアップまで全自動で完了します：

```bash
git clone https://github.com/UrtYottaro/Token_Reader.git ~/Token_Reader && cd ~/Token_Reader && bash setup.sh
```

> `git clone` を使うためGatekeeperの警告は発生しません。
> インストール先を変更したい場合は `~/Token_Reader` を任意のパスに変更してください。

### 方法2: ダブルクリックでセットアップ

ZIPでダウンロードした場合は `セットアップ.command` をダブルクリックしてください。

> **初回ダウンロード時の注意（Gatekeeper警告）**
>
> macOSのセキュリティ機能により「開けません」と表示される場合があります。
> `セットアップ.command` を**右クリック → 開く** で実行してください。
> （「開く」ボタンが表示されるので、それをクリック）
>
> 一度開けば、次回以降は警告なしでダブルクリックで実行できます。

### 方法3: Terminalから手動実行

```bash
cd ~/Token_Reader   # ← 実際の配置場所に変更
bash setup.sh
```
...
├── セットアップ.command      # ダブルクリックでセットアップ
├── アンインストール.command   # ダブルクリックでアンインストール
