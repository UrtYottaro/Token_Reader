...
### 方法1: ダブルクリックで簡単セットアップ（推奨）

`セットアップ.command` をダブルクリックしてください。
...
> 以下のいずれかの方法で解除してください：
>
> **方法A（推奨）：** ダウンロード後にTerminalで以下を1回だけ実行
> ```bash
> xattr -cr ~/Downloads/Token_Reader-main   # ← 展開先のパスに変更
> ```
>
> **方法B：** `セットアップ.command` を**右クリック → 開く** で実行
